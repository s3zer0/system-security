        ...
        for vuln in data['trivy']['vulnerabilities']:
            ...
            if package_name in data['gpt5_results']:
                if version in data['gpt5_results'][package_name]:
                    mapping = data['gpt5_results'][package_name][version]['mapping_result']
                    if cve_id in mapping:
                        vulnerable_apis = mapping[cve_id]['vulnerable_apis']
...
if __name__ == "__main__":
    ...
    data_dir = sys.argv[1] if len(sys.argv) > 1 else "../DB"
    ...
    file_paths = {k: os.path.join(data_dir, v) for k, v in required_files.items()}
    
    # 파일 존재 여부 검증
    missing_files = [v for v in file_paths.values() if not os.path.exists(v)]
    if missing_files:
        print(f"오류: 필수 파일을 찾을 수 없습니다:")
        for f in missing_files:
            print(f"  - {f}")
        print(f"\n사용법: python3 main.py [데이터_디렉토리]")
    ...
    results = evaluator.run_analysis(
        ast_file=file_paths["ast_file"],
        gpt5_results_file=file_paths["gpt5_results_file"],
        lib2cve2api_file=file_paths["lib2cve2api_file"],
        trivy_file=file_paths["trivy_file"],
        output_file=os.path.join(data_dir, "patch_priorities.json")
    )
    ...
    modules = results.get('modules_by_priority', [])
    for i, module in enumerate(modules[:5], 1):  # 상위 5개 모듈 표시
        external_badge = " 🚨 DOCKER 외부 노출" if module.get('docker_external_exposure', False) else ""
        print(f"\n{i}. {module.get('package_name', '알 수 없음')} v{module.get('current_version', '알 수 없음')}{external_badge}")
        print(f"   우선순위: {module.get('priority_level', '알 수 없음')}")
        print(f"   위험 점수: {module.get('risk_score', 0)}/100")
        print(f"   취약점 수: {len(module.get('vulnerabilities', []))}")
        
        patching = module.get('patching', {})
        if patching:
            print(f"   수정 방법: {patching.get('upgrade_command', '해당 없음')}")
        
        recommendation = module.get('overall_recommendation', '')
        if recommendation:
            print(f"   권장사항: {recommendation[:150]}...")